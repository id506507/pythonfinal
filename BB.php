<?php
	...
	if ( !( $result = mysqli_query($database,"SELECT * FROM bb") ) )//是否可以query
         {
            print( "<p>Could not execute query!</p>" );
            die( mysql_error() . "</body></html>" );
         }
    $result=mysqli_query($database,"SELECT content FROM bb ORDER BY at DESC"); //抓DB裡的data
    $result2=mysqli_query($database,"SELECT at FROM bb ORDER BY at DESC"); //抓DB裡的data
				...
				<div class="single-page">
						<div class="col-md-8 single-gd-lt">
							<div class="single-pg-hdr">
								<h2>BIGBANG</h2>
								<p>YG娛樂於2006年推出的韓國男子音樂組合，由T.O.P、太陽、G-Dragon、大聲及勝利五名成員組成，G-Dragon則擔任隊長一職。</p>
							</div>
						</div>
						<div class="col-md-4 single-gd-rt">
						<img src="images/BB.jpg">
						</div>
						同期競爭對手：C.N.BLUE、太妍、SHINee
						<div class="clearfix"></div>
...
MANILA:{
        center:{lat:14.599320, lng:120.984612}
      },
JAKARTA:{
        center:{lat:-6.211980, lng:106.847358}
      },
KUALA_LUMPUR:{
        center:{lat:3.143303,lng: 101.689733}
      },
上海:{
        center:{lat:31.237225,lng: 121.482072}
      },
長沙:{
        center:{lat:28.226676,lng: 112.943528}
      },
成都:{
        center:{lat:30.567222,lng: 104.067263}
      }
   };
function initMap() {
