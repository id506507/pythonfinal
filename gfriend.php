<?php
	...
	if ( !( $result = mysqli_query($database,"SELECT * FROM gfriend") ) )//是否可以query
         {
            print( "<p>Could not execute query!</p>" );
            die( mysql_error() . "</body></html>" );
         }
    $result=mysqli_query($database,"SELECT content FROM gfriend ORDER BY at DESC"); //抓DB裡的data
    $result2=mysqli_query($database,"SELECT at FROM gfriend ORDER BY at DESC"); //抓DB裡的data
				...
				<div class="single-page">
						<div class="col-md-8 single-gd-lt">
							<div class="single-pg-hdr">
								<h2>GFRIEND</h2>
								<p>Source Music於2015年推出的韓國六人女子組合，成員包括Sowon、Yerin、Eunha、Yuju、SinB與Umji，由Sowon擔任隊長。</p>
							</div>
						</div>
						<div class="col-md-4 single-gd-rt">
						<img src="images/gfriend.jpg">
						</div>
						同期競爭對手：GOT7、NCT 127、SHINee
						<div class="clearfix"></div>
...
盆唐:{
        center:{lat:37.378949,lng: 127.105759}
      },
永登:{
        center:{lat:37.520902,lng: 126.909464}
      },
水原:{
        center:{lat:37.286508,lng: 127.036918}
      },
九宜:{
        center:{lat:37.671718, lng:128.025545}
      },
龍山:{
        center:{lat:37.528851,lng: 126.966344}
      },
日山:{
        center:{lat:35.497108,lng: 129.422826}
      },
鷺梁津:{
        center:{lat: 37.510106,lng: 126.943654}
      },
/*北京:{
...
釜山:{
        center:{lat:35.233477,lng: 129.079904}
      },
TAIPEI:{
