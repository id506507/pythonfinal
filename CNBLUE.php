<?php
	...
	if ( !( $result = mysqli_query($database,"SELECT * FROM cnblue") ) )//是否可以query
         {
            print( "<p>Could not execute query!</p>" );
            die( mysql_error() . "</body></html>" );
         }
    $result=mysqli_query($database,"SELECT content FROM cnblue ORDER BY at DESC"); //抓DB裡的data
    $result2=mysqli_query($database,"SELECT at FROM cnblue ORDER BY at DESC"); //抓DB裡的data
				...
				<div class="single-page">
						<div class="col-md-8 single-gd-lt">
							<div class="single-pg-hdr">
								<h2>C.N.BLUE</h2>
								<p>韓國FNC娛樂旗下的四人樂團組合，成員包括鄭容和（隊長）、李宗泫、姜敏赫、李正信。</p>
							</div>
						</div>
						<div class="col-md-4 single-gd-rt">
						<img src="images/CNBLUE.jpg">
						</div>
						同期競爭對手：BIGBANG、NCT 127、太妍
						<div class="clearfix"></div>
...
Yokohama:{
        center:{lat:35.459972,lng: 139.575989}
      },
Kokuritsudai:{
        center:{lat:35.458777,lng: 139.636975}
      },
釜山:{
