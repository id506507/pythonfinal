<?php
	...
	if ( !( $result = mysqli_query($database,"SELECT * FROM shinee") ) )//是否可以query
         {
            print( "<p>Could not execute query!</p>" );
            die( mysql_error() . "</body></html>" );
         }
    $result=mysqli_query($database,"SELECT content FROM shinee ORDER BY at DESC"); //抓DB裡的data
    $result2=mysqli_query($database,"SELECT at FROM shinee ORDER BY at DESC"); //抓DB裡的data
				...
				<div class="single-page">
						<div class="col-md-8 single-gd-lt">
							<div class="single-pg-hdr">
								<h2>SHINee</h2>
								<p>為韓國SM娛樂旗下的五人男子組合，成員包括溫流、鐘鉉、KEY、珉豪及泰民。團體名「SHINee」，「Shine」名詞型，語尾再加上「ee」組合而成的新名詞，具有「受光芒照耀之人」的決心意義。</p>
							</div>
						</div>
						<div class="col-md-4 single-gd-rt">
						<img src="images/SHINee.jpg">
						</div>
						同期競爭對手：太妍、NCT 127、BIGBANG
						<div class="clearfix"></div>
...
桃園:{
        center:{lat:25.079715,lng: 121.234126}
      },
羽田機場:{
        center:{lat:35.549406,lng: 139.779812}
      },
成田機場:{
        center:{lat:35.772069,lng: 140.392684}
      },
香港:{
